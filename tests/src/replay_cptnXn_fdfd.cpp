...
TEST_CASE("cptnXn_fdfd")
{
    ...
    //            parser.from_file(filename, lzma_enabled),
    //            parser.from_string(file_string(filename), lzma_enabled));
    //
    //    REQUIRE(rp_e);
    //    const auto& r = *rp_e;
    //
...
TEST_CASE("replay without frames")
{
    ...
    const auto rp_e = GENERATE_REF(
            parser.from_file(filename, false),
            parser.from_string(file_string(filename), false));
